...
main() {
    ...
    say "       · 下面一块是**标签页组**，底部有四个标签，点一下就能切换："
    say "         「订单列表」「超时订单」「中转站 / 集散」「停靠明细」"
    say ""
    printf '  %s④ 日志（窗口最底部）%s\n' "$BOLD" "$RESET"
    ...
    want "共 106 条有向边（56 条道路，其中 6 条是**单向**的）"
    ...
    want "触发重规划后紧跟一行「→ 增量式重规划：仅重算受影响的路段，其余原样保留」"
    want "  （若上一版是多趟方案，则显示「→ 上一版为多趟方案，退回全量重算」）"
    ...
    # ---------------------------------------------------------------- 第 16 关
    stage "多趟配送与中转集散（E4 / D21）"
    say "本关用**默认配置**验证——默认载重已调低到 200kg，多趟与集散是常规行为。"
    say ""
    say "先说明：第 15 关结束时关掉了程序，这里重新启动，第 17、18 关共用这一次会话。"
    if ! launch_app; then
        record "多趟配送与中转集散（E4）" "失败" "程序未能启动"
        summary
        exit 1
    fi
    sleep 2
    ...
    say ""
    head1 "预期（看右侧「路线信息」）："
    want "**不显示「不可行」**，而是「停靠 25 站，已送达 0 站，共 N 趟」（N 明显大于 1）"
    want "多趟时下面会逐趟列出，形如："
    want "  第 1 趟：W01 → T01（2 节点，约 X km）"
    want "    入库暂存 T01 约 190kg"
    want "  第 2 趟：T01 → T01（若干节点）"
    want "    取货配发 T01 约 190kg"
    want "即「补给入库」与「取货二次配发」交替出现"
    say ""
    todo "点右侧栏的「中转站 / 集散」标签页"
    want "列出 3 个中转站：中转站 | **子网络** | **下属配送点** | 峰值暂存 | 当前暂存"
    want "子网络为 1/2/3，下属配送点为 9/7/9 —— 对应「中转站A下属若干配送点」"
    want "**三个中转站的峰值暂存都非 0**（说明集散机制确实在工作）"
    want "**当前暂存初始为 0**（车还没出发，站里没货）"
    say ""
    dim "本关不要求你推进；下一关会顺带看到「当前暂存」随推进变化。"
    pause
    ask_result "多趟配送与中转集散（E4）"

    # ---------------------------------------------------------------- 第 17 关
    stage "单向路与有向性（D18）"
    say "程序仍在运行（与上一关同一次会话）。"
    say ""
    todo "回到画布，找一条**只有一个箭头**的边（全图共 6 条这样的单行道）"
    say ""
    head1 "预期："
    want "绝大多数边两端各有一个箭头（双向路）"
    want "但至少有 6 条边**只有一端有箭头**——这就是单行道，无法逆行"
    want "可以先点「手工增删…」删掉某条边的反向，观察高亮路线是否绕行"
    say ""
    head1 "命令行也可以核对（向导直接跑给你看）："
    printf '    %s\n' "$(QT_QPA_PLATFORM=offscreen "$APP" --dump-graph list 2>/dev/null | head -2)"
    dim "完整清单可用后面「图表示输出」那一关的方式打开文件查看。"
    pause
    ask_result "单向路与有向性（D18）"

    # ---------------------------------------------------------------- 第 18 关
    stage "增量式重规划与「当前暂存」随推进（D22）"
    say "程序仍在运行（与第 16 关同一次会话）。"
    say ""
    todo "点几次工具栏的「推进一站」，观察右侧「中转站 / 集散」面板的「当前暂存」列"
    say ""
    head1 "预期："
    want "「当前暂存」是**截至当前位置**的站内货量，会随推进**变化**"
    want "  （第 1 趟到 T01 卸货后应为非 0；第 2 趟取货后回到 0）"
    want "已走过的路段**不再标红**，只有尚未走完的路线高亮"
    say ""
    todo "再点几次「模拟路况」，看底部日志"
    say ""
    head1 "预期："
    want "当出现「受影响边位于当前路径且增幅达标 -> 自动触发重规划」时，"
    want "  紧跟一行「**→ 增量式重规划：仅重算受影响的路段，其余原样保留**」"
    want "若上一版是多趟方案，则显示「→ 上一版为多趟方案，退回全量重算」"
    want "两种都算通过——重点是程序**明确说明了采用了哪种方式**"
    pause
    ask_result "增量式重规划与当前暂存（D22）"
    stop_app
    pause
