...
main() {
    ...
    want "「路线信息」：总距离约 262.2 km、总成本约 290.8 元、停靠 25 站"
    pause
    ...
    want "总距离变大到约 288.2 km，总成本变小到约 288.2 元"
    ...
    say ""
    head1 "预期（看底部日志）："
    want "新增一行「路况变化：改动 N 条边」（N 约为 72 的 10%，即 7 条）"
    ...
    say ""
    head1 "预期："
    want "日志出现「插入紧急订单 <订单号> @ <配送点>」"
    want "该配送点在新的高亮路线里被**优先服务**（排在前面的停靠点）"
    ...
    todo "点「添加边」"
    want "日志「手工添加边 D01 -> D25」；画布出现该边并重新规划"
    say ""
    printf '  %s② 删除边%s\n' "$BOLD" "$RESET"
    todo "用同一对 ID 点「删除边」"
    want "日志「手工删除边 D01 <-> D25」；该边（含反向）消失"
    say ""
    ...
    todo "点工具栏「推进一站」，连点 4–5 次。"
    todo "然后点「重新规划」。"
    say ""
    head1 "预期："
    want "每次推进，日志记录「送达 XX（到达 时:分）」"
