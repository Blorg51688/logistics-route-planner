...
main() {
    ...
    # ---------------------------------------------------------------- 第 16 关
    stage "多趟配送与中转集散（E4 修订后）"
    say "本关用一份「载重被改小」的配置启动程序。"
    say ""
    dim "注意：按新的口径（设计 D21），总需求超过载重**不再是"不可行"**——"
    dim "车辆会反复返回仓库取货、把货暂存在中转站、再二次配发。"
    say ""
    mkdir -p build
    ...
    if confirm "现在用该配置启动程序？"; then
        if launch_app --config build/test_capacity.ini; then
            ...
            say ""
            head1 "预期："
            want "「路线信息」**不再显示"不可行"**，而是「停靠 25 站，已送达 0 站，共 N 趟」（N 远大于 1）"
            want "多趟时下面会逐趟列出，形如："
            want "  第 1 趟：W01 → T01（2 节点，7.6km）"
            want "    入库暂存 T01 90kg"
            want "  第 2 趟：T01 → T01（5 节点，14.6km）"
            want "    取货配发 T01 90kg"
            want "即"补给入库"与"取货二次配发"交替出现"
            say ""
            todo "点右侧栏的「中转站 / 集散」标签页"
            want "列出 3 个中转站，每行是：中转站 | **子网络** | **下属配送点** | 峰值暂存 | 当前暂存"
            want "子网络列为 1/2/3，下属配送点为 9/7/9 —— 对应"中转站A下属若干配送点""
            want "**峰值暂存非 0**（说明中转站确实参与了集散）、**当前暂存为 0**（不留残余库存）"
            want "画布上仍有红色高亮路线，程序不崩溃"
            pause
        else
            record "多趟配送与中转集散（E4）" "失败" "未能用测试配置启动程序"
            summary
            exit 1
        fi
    ...
    # ---------------------------------------------------------------- 第 18 关
    stage "增量式重规划（D22）"
    say "本关验证路况触发的重规划是**增量式**的（只重算受影响的路径段）。"
    say ""
    todo "点工具栏的「模拟路况」，多点几次，看底部日志。"
    say ""
    head1 "预期："
