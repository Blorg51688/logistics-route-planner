...
main() {
    ...
    want "    第 1 趟：W01 → T01（2 节点，约 X km）  入库暂存 T01 约 190kg"
    want "    第 2 趟：T01 → T01（若干节点）        取货配发 T01 约 190kg"
    want "  「补给入库」与「取货二次配发」交替出现"
    say ""
    ...
    want "初始时「当前暂存」为 0、「峰值暂存」也为 0（车还没出发）"
    todo "再切到「停靠明细」标签页"
    want "每个配送点一行：配送点 | 原始到达 | 等待 | 送达 | 离开 | 剩余载重"
    pause
    ask_result "规划结果、多趟与中转集散（B6/B7/E4）"
    ...
    want "日志区分「送达 XX（到达 时:分）」与「经过 XX」，到中转站时补注「入库暂存 / 取货配发」"
    say ""
    todo "切到「中转站 / 集散」标签页，继续推进"
    want "「当前暂存」随推进**实时变化**（第 1 趟到 T01 卸货后非 0，第 2 趟取货后回落）"
    want "「峰值暂存」是**实时刷新的**——只有真的存进去且超过历史峰值时才变大"
    say ""
    todo "一直推到全部送完"
