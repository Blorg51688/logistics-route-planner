...
main() {
    ...
    dim "     （若某个按钮找不到，多半是窗口没最大化；见下面的排障提示）"
    ...
    want "    第 1 趟：W01 → …（若干节点）"
    want "  **初始规划下各趟都是直接从仓库出发、送完返回仓库**——不绕中转站"
    say ""
    ...
    want "**初始规划下两列都是 0** —— 车还没从簇里带回任何东西"
    say ""
    todo "推进若干站，再点「插入紧急订单」触发一次重规划，回头看这个面板"
    want "**「当前暂存」可能变成非 0** —— 这是「顺路寄存」在起作用："
    want "  车本次要回仓库，而车上还载着**来不及送**的货，且该货所属簇的中转站"
    want "  就在回程路径上，于是顺手卸在站里（**零成本**，不绕路）"
    want "一旦出现存货，它会被**保留**下来，供后续规划把它当前置仓库使用（积少成多）"
    say ""
    dim "看不到非零存货**不一定是错**：寄存只在「来不及送 + 站在回程路上」两个条件"
    dim "同时满足时才发生。条件不满足就不寄存，这是设计意图（不为用站而绕路）。"
    say ""
    todo "一直推到全部送完"
